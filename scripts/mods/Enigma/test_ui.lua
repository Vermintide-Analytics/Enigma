local definitions = local_require("scripts/mods/Enigma/test_ui_definitions")
local DO_RELOAD = true
EnigmaTestUI = class(EnigmaTestUI)

local enigma = get_mod("Enigma")

EnigmaTestUI.init = function(self, parent, ingame_ui_context)
	self._parent = parent
	...
	self.colors = {
		...
		selected = Colors.get_table("online_green")
	}
end

EnigmaTestUI.create_ui_elements = function (self)
	DO_RELOAD = false
	self.ui_scenegraph = UISceneGraph.init_scenegraph(definitions.scenegraph_definition)
	self.countdown_widget = UIWidget.init(definitions.widgets.fullscreen_countdown_2)

	UIRenderer.clear_scenegraph_queue(self.ui_renderer)
end

EnigmaTestUI.update = function (self, dt)
	if DO_RELOAD then
		self:create_ui_elements()
...
EnigmaTestUI.is_enter_game = function (self)
	return self._countdown_active
end

EnigmaTestUI.draw = function (self, dt)
...
EnigmaTestUI.update_enter_game_counter = function (self, start_time, max_start_time, dt)
...
EnigmaTestUI.play_sound = function (self, event)
	WwiseWorld.trigger_event(self.wwise_world, event)
end

EnigmaTestUI._get_start_time = function (self)
    return enigma.current_start_time, enigma.max_start_time
end